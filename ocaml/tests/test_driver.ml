...
class fake_slave config : Python.slave =
  ...
  object (_ : #Python.slave)
    ...
    method invoke_async request ?xml parse_fn =
      ignore xml;
      match request with
      | `List [`String "download-and-import-feed"; `String url] -> A.make_fulfilled @@ parse_fn @@ download_and_import url
      | `List [`String "get-distro-candidates"; `String url] -> A.make_fulfilled @@ parse_fn @@ get_distro_candidates url
      | _ -> raise_safe "Unexpected request %s" (Yojson.Basic.to_string request)

    method run_async : 'a. 'a A.promise -> unit = fun promise -> ignore @@ A.toplevel_wait_for promise; ()
    method close = ()
...
let suite = "driver">::: [
  "simple">:: Fake_system.with_tmpdir (fun tmpdir ->
    ...
    let promise = Driver.solve_with_downloads config fetcher distro reqs ~force:true ~update_local:true in
    let (ready, result) = A.toplevel_wait_for promise in
    if not ready then
