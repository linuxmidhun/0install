    ...
    | F.PackageImpl info ->
        if info.F.package_installed then (`Null, "(package)", "This distribution-provided package is already installed.")
          ...
          let size =
            ...
            | None -> None
            | Some attrs ->
                try Some (Yojson.Basic.Util.to_number (List.assoc "size" attrs))
                with Not_found -> None in
          match size with
...
let get_selections_gui config (slave:Python.slave) ?test_callback distro ?(systray=false) mode reqs ~refresh ~use_gui =
  if use_gui = No then `Dont_use_GUI
    ...
    let get_bug_report_details = function
      | [] -> Lwt.return (
      ...
      | json -> raise_safe "get_bug_report_details: invalid request: %s" (Yojson.Basic.to_string (`List json)) in
    Python.register_handler "get-bug-report-details" get_bug_report_details;

    let get_component_details = function
      | [`String uri] -> Lwt.return (`Assoc (
          ("feeds", `List (list_feeds !feed_provider uri)) ::
          list_impls config (snd !results) uri
        );
      )
      | json -> raise_safe "get_component_details: invalid request: %s" (Yojson.Basic.to_string (`List json)) in
    Python.register_handler "get-component-details" get_component_details;

    let justify_decision = function
      ...
      | json -> raise_safe "justify_decision: invalid request: %s" (Yojson.Basic.to_string (`List json)) in
    Python.register_handler "justify-decision" justify_decision;
    ...
    let run_test = function
      ...
      | json -> raise_safe "run_test: invalid request: %s" (Yojson.Basic.to_string (`List json)) in
    Python.register_handler "run-test" run_test;
